...
def color(tree, cf):
	...
	for sym0 in syms:
		if sym0['heapify']:
			...
			chain		= node['chain']
			interference	= toColors(sym0['interferenece'])
			preference	= not sym0['span-funcall']
			
			if isinstance(chain, PhiChain):
				#Here we will try and give all phi-related symbols the same
				#color.
				
				colors = sym0.getColors()
				...
				if not color:
					...
					# FIXME: Check to see if we can latch on to any related chains.
					...
					oldChain = chain
					chain = chain.split(sym0)
					
					for sym1 in sym0['related']:
						if sym1['chain'] != oldChain and sym1['chain'].last() == sym1:
							# FIXME: See if we can latch on to the OTHER related chain.
